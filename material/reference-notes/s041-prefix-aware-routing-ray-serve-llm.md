# S041 笔记：Prefix-aware routing — Ray Serve LLM
...
- 日期：`current`
- 类型：`official docs`
- 本地材料：`cited-materials/s041-prefix-aware-routing-ray-serve-llm.pdf`
- 提图状态：`completed`
- 可用图片数：`0`
...
prefix cache 之后的工程化 routing 权衡。

更具体地说，这篇材料对“agentic AI 推理负载如何改变 AI CPU 职责”的启发主要有三层：
1. 它说明了什么问题正在从 GPU 内部计算转移为 host/control-plane 问题：prefix cache 之后的工程化 routing 权衡。
2. 它给出了哪类硬证据或定量迹象：`match_rate_threshold` 默认 `0.1`；高匹配走 affinity；失衡时回退 P2C。
3. 它对工程判断的意义在于：CPU 不只是陪跑，而是在请求排队、状态放置、缓存复用、阶段切换或跨池协调中承担持续决策责任。

## 关键证据

- 主要用途：prefix cache 之后的工程化 routing 权衡
- 关键证据/数据：`match_rate_threshold` 默认 `0.1`；高匹配走 affinity；失衡时回退 P2C
- 建议回看原文时优先关注：`s041-prefix-aware-routing-ray-serve-llm.pdf` 中与 `D12` 对应的图、表或系统示意。

## 图文笔记

### 图：`s041-page1.png`

![s041-page1.png](../../assets/reference-note-fallbacks/s041-page1.png)
...
- 对主线判断的贡献：prefix cache 之后的工程化 routing 权衡。
