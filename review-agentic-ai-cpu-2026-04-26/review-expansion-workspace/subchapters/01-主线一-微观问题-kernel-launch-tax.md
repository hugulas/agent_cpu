...
这张图的价值在于：它把“CPU 只是 GPU 的陪跑”这一常见误解直接可视化。输入预处理（包括 tokenization）、HTTP 请求处理、kernel launch 三件事虽然都不是矩阵乘法，但任何一件掉队，GPU 就会空等。**本章聚焦的是第三条——kernel launch 及其在 serving 中的放大效应**，因为 tokenization 等预处理属于“请求进入前的准备阶段”，而 launch tax 属于“算子下发阶段”；两者虽然都走 CPU，但在系统优化路线上的位置不同。

#### 2.2 Tokenization：作为背景，不是本章焦点

Tokenization 确实是 agentic 场景下显著的 CPU 负载来源——S005 测量表明，在 Llama 3.1 8B（4×H200）上它可占 TTFT 的 **up to 50%**，且随上下文长度线性增长。但这一瓶颈属于**请求预处理阶段**，优化路线（Rust 多线程 tokenizer、预 tokenization、缓存）与 kernel launch tax 的优化路线（CUDA Graphs、编译器图化、persistent batch）完全不同。因此本章不再展开 tokenization 的细节机制；需要了解其量化轮廓的读者可参考 S005 Figure 5 的原始测量。[1]

#### 2.3 HTTP 请求处理： query rate 决定可见度

HTTP server 负责连接管理、请求解析和批量调度。S005 指出，这类开销随 query rate 上升而非模型大小上升，通常在 500 RPS 以上才会与 tokenization 成本可比。因此，在大多数分析中，**kernel launch 是算子下发路径上更主要的 CPU 瓶颈来源**，而 HTTP 和 tokenization 属于前置/外围路径。
...
#### 2.7 补充洞察：从 kernel launch 到 GPU stall 的级联放大

把 S005 的 Figure 2 和 Figure 13 并置阅读，会发现它们不是两个孤立的发现，而是同一条因果链上的两个观测点：

| 观测点 | 揭示的问题 |
| --- | --- |
| **Fig. 2（推理管线全局）** | CPU 负责调度、同步和 launch，是 GPU 计算的前置条件；CPU 慢则整条管线慢 |
| **Fig. 13（多 GPU 同步链）** | 当 host CPU 因 launch 调度延迟而变慢时，共享内存广播队列的 dequeue 从 12 ms 放大到 228 ms（19×），成为整条链最慢的环节 |

**这条链的深层含义是：kernel launch tax 不是孤立税费，而是会在推理管线的不同阶段之间产生级联阻塞。**

- Host CPU 的 launch 线程被 OS 调度延迟（§2.4）→
...
1. **阶段切换更频繁** → scheduling、broadcast、launch 被反复触发，累积成本不是线性而是超线性增长；
2. **Batch shape 更不稳定** → 无法通过一次性 warm-up 或静态图化把所有控制路径都 capture 住，host CPU 必须在每个 step 参与 launch 和同步决策；
3. **工具间隙更密集** → 每一次 resume 都要重新进入调度链，launch tax 被反复征收。

因此，Figure 2 和 Figure 13 共同指向一个比“CPU 有点慢”更强的结论：

> **在多 GPU LLM serving 中，CPU 已经从“把请求送进去就完成任务”的辅助角色，转变为持续参与每一阶段推进的结构性瓶颈。kernel launch tax 只是这个瓶颈链的微观入口，但通过同步边界它可以被放大到决定系统上限的程度。**
