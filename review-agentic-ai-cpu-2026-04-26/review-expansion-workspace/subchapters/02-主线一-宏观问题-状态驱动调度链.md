...
| 判断 | 直接支撑材料 | 关键数字或图 |
| --- | --- | --- |
| 调度墙来自一整条 host-side 状态链，而不是单个 launch | S005 (CPU slowdown); S021 (PD disaggregation); **S052** (Sutradhara) | `12ms -> 228ms` dequeue amplification；工具调用占 FTR `30%`~`85%`；PD 分离结构图；orchestrator-engine co-design 图 |
| queue、worker selection、handoff、synchronization 会顺序放大彼此抖动 | S005 (CPU slowdown); S021 (PD disaggregation); **S052** (Sutradhara) | `2603.22774_sync-barrier.png`；`s021-prefill-decode-disaggregation_page_0002-2.png`；`2601.12967_sutradhara-parallel-design.png` |
| agentic workload 会把状态切换密度推高，因此更容易把 CPU 推到关键路径 | S021 (PD disaggregation); **S052** (Sutradhara) | prefill compute-bound / decode memory-bound；显式阶段切换与跨角色 handoff；streaming tool dispatch 解锁 intra-request parallelism |
...
这一判断还得到了 agent serving 系统的直接验证。**Sutradhara**（S052）通过大规模生产 trace 分析发现，在工具型 agentic 推理中，**工具调用延迟占 FTR（First Token Rendered）延迟的 `30%`~`85%`**。其根因不是工具本身慢，而是 orchestrator 与 LLM engine 作为**解耦黑盒**运行：orchestrator 掌握迭代边界和工具依赖信息，engine 控制调度和 KV cache，两者互不利用对方信息做全局最优决策。[3] 这意味着调度墙的一部分不是来自 engine 内部的 launch 开销，而是来自**跨层信息壁垒导致的串行化**——orchestrator 必须等 engine 完整 decode 才能 dispatch 工具，engine 必须等工具返回才能启动下一轮 prefill，这种"等-做-等"的串行模式把 host CPU 固定在状态推进的关键路径上。
...
第三段是 **Sutradhara**（S052）揭示的 orchestrator-engine 架构错配。生产 trace 分析表明，当前 agentic 系统把编排器和推理引擎当作独立黑盒，仅通过不透明的 request-response 接口通信。orchestrator 知道工具何时可被并行派发，但 engine 必须等待完整 decode 结束才能交出输出；engine 知道 KV cache 的命中状态，但 orchestrator 无法据此指导调度。这种信息壁垒直接导致：每一次工具调用都引入一次完整的"decode 结束 → 输出解析 → 工具执行 → 结果回填 → 重新 prefill"串行链，FTR 延迟的 `30%`~`85%` 被浪费在这条链上。[3]
...
这些特征共同带来的结果是：CPU 参与的状态动作次数明显增加，而每次动作的收益窗口却更短。`S021`（BentoML PD disaggregation 文档）给出了阶段异构性、跨角色切换和显式 handoff 这些结构性原因；`S005` 证明了同步放大效应；而 **Sutradhara** 则进一步量化了 agentic 场景下跨层串行的代价——工具调用间隙不是"不可避免的等待"，而是**信息壁垒造成的人为串行化**。[1][2][3]

Sutradhara 通过三项优化打破了这种串行化：
1. **Prompt Splitting**：在工具执行期间，利用工具无关上下文投机性开始下一轮 prefill，工具输出到达后增量扩展，单请求延迟降低最高 `42%`；
2. **Streaming Tool Dispatch**：在 decode 流中实时识别完整工具调用 JSON 并立即 dispatch，无需等待完整输出；
3. **Orchestrator-Aware Cache Management**：通过语义标签指导 engine 的 eviction 和调度。

这三项优化的共同前提是：orchestrator 与 engine 必须共享信息。只有当 CPU 控制面同时掌握"迭代边界、工具依赖、prompt 组合"和"调度状态、KV cache 位置、batch 特征"，才能把串行链压缩成并行流水线。[3] 这直接说明：**调度墙的一部分可以通过控制面语义化来拆除，而不仅仅是通过减少 kernel launch 次数。**
...
> 在 agentic inference 中，CPU 进入关键路径的主要方式，是把原本分散的 queue、selection、handoff 和 synchronization 串成一条高频状态驱动调度链；GPU 计算只要略微变短或略微被切碎，这条链就会快速变成系统上限。而当这条链被 orchestrator-engine 信息壁垒人为串行化时，即使每个环节的局部延迟都不大，跨层等待的累积也足以主导端到端结果。

`12ms -> 228ms` 的 dequeue 放大、PD 分离下的显式角色切换，以及 Sutradhara 量化的 `30%`~`85%` FTR 工具延迟占比，共同说明调度墙的本质不是某一个 launch 过慢，而是 host-side 状态链——包括跨层信息壁垒——已经开始主导系统上限。[1][2][3]
...
[3] [Sutradhara: An Intelligent Orchestrator-Engine Co-design for Tool-based Agentic Inference](../../../material/reference-notes/s052-sutradhara-orchestrator-engine-codesign.md). 2026-01-19.
