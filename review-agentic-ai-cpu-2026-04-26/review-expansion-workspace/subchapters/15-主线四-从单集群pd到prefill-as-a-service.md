...
| 判断 | 直接支撑材料 | 关键数字或图 |
| --- | --- | --- |
| PD 分离已经从“单集群角色拆分”升级为“跨池状态编排” | `S001 (Prefill-as-a-Service) S002 (K8s disaggregated LLM) S003 (NVIDIA Dynamo agentic) S015 (LMCache disaggregated prefill) S023 (PD disaggregation review)`；**S052** (Sutradhara) | throughput `+54%`；P90 TTFT `-64%`；prompt splitting 跨阶段重叠；相对朴素异构基线 `+32%` |
...
| agentic workload 把 KV handoff、pool selection、远端回传和带宽取舍推成 CPU 的核心职责 | `S001 (Prefill-as-a-Service) S003 (NVIDIA Dynamo agentic)`；**S052** (Sutradhara) | KV-aware placement；priority scheduling；orchestrator-engine 五 API 协同；跨池 / 跨域 Prefill-as-a-Service |
...
![Disaggregated prefill/decode overview](../assets/subchapters/15/vllm-disagg-prefill-overview.jpg)
...
**Sutradhara**（S052）从另一个角度验证了这种跨阶段解耦的工程可行性。它提出的 **prompt splitting** 机制表明：在第 i 轮工具执行期间，第 i+1 轮的工具无关上下文可以被投机性地提前 prefill，工具输出到达后再增量扩展。这一优化将串行的"decode → 工具执行 → prefill"链压缩成可重叠的流水线，单请求延迟降低最高 `42%`，同负载下系统可支撑 `77%` 更高的 QPS。[6] 这说明，一旦 prefill 被拆成独立阶段（无论是本地 PD 还是远端 PraaS），它就具备了与工具执行**并行化**的潜力——PraaS 不仅解决资源错配，还为跨阶段流水线打开了优化空间。

### 图 2：PraaS 的核心不是多一个池，而是多一层跨池协调层

![Disaggregated serving on Kubernetes](../assets/subchapters/15/nvidia-k8s-disagg-serving-2026.webp)
...
5. 跨层语义协同。orchestrator 必须向 engine 传递 tool dependency、prefix reuse hint 和 cache metadata，engine 必须向 orchestrator 回传 cache state，才能把分布式状态编排从"黑盒猜测"变成"白盒协同"。

**Sutradhara 的五 API 接口层**（`hint_tool_dependency`、`hint_prefix_reuse`、`prompt_split`、`stream_dispatch`、`report_cache_state`）直接证明了这种跨层协同不是可选优化，而是 agentic 推理的功能性前提。[6] 在没有这种协同的系统中，即使物理上把 prefill 和 decode 拆到不同节点，信息壁垒仍会把它们串行化；有了协同，分布式阶段才能真正并行。
...
从单集群 PD 到 Prefill-as-a-Service 的跃迁，本质上是把“阶段切分问题”升级成“分布式状态与服务编排问题”。单集群 PD 解决资源错配，PraaS 解决更高层级的状态位置与阶段位置协同，而 Sutradhara 的 prompt splitting 与 streaming dispatch 则证明跨阶段并行化在工程上是可行的。吞吐 `+54%`、P90 TTFT `-64%`、异构基线 `+32%` 以及单请求 `-42%` 延迟改善的结果，共同说明这不是概念包装，而是 AI CPU 职责边界被继续向外推的明确信号。下一章再进一步收束：这会具体把 CPU 的设计目标推向哪些方向。[1][2][3][4][5][6]
