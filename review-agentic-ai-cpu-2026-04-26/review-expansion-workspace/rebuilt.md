...
| 判断 | 直接支撑材料 | 关键数字或图 |
| --- | --- | --- |
| agentic workload 的 KV 访问已从 write-heavy 转为 read-dominant，CPU 恢复动作的频率远超写入 | `S003 (NVIDIA Dynamo agentic)` | read/write ratio `11.7x` |
...
| 工业 serving 栈已将 retention、event API 和 token-range 策略硬化为 CPU 控制面职责 | `S034 (TensorRT-LLM KV reuse)` | priority-based eviction；KV event API |
...
**第一层：状态 keeper 的计算负载。** CPU 需要维护哪些高价值块应继续驻留、哪些块已过期。这不再是简单的 LRU，而是需要按业务价值（token range、session 优先级、复用概率）做加权判断。TensorRT-LLM 的 priority-based eviction 和 pinned prefix 需求说明，CPU 的保留策略已经从"统一驱逐"变成"差异化治理"。[4]
...
本节要建立的不是一个新名词，而是一条更准确的因果链：当 agentic workload 让 KV 呈现 `write-once-read-many`、高频 resume 和跨阶段复用特征时，KV 就会从"容量补丁"变成"生命周期对象"。这个转变的直接后果是 CPU 职责的三层扩展：从 state keeper（差异化保留策略）到 recovery planner（跨层级恢复决策）再到 prefetch trigger（layer-ahead 流水线协同）。读写比 `11.7x`、NOSA 的 `2.3x` 吞吐提升、ScoutAttention 的 `2.1x` 预取收益，以及 TensorRT-LLM 的 retention/event API 共同说明，CPU 的新职责不是把 KV 存下，而是把它留住、找回、复用并以更低代价重新送回关键路径。[1][2][3][4]
...
图 1 是工业级验证：当状态复用被系统化实施后，首 token 延迟可以被大幅压低。它支撑本节的核心判断：APC 的价值不是"平均省下一点算力"，而是把状态复用正式引入了服务系统的关键路径。[2]
...
![NVIDIA Dynamo KV-aware placement](assets/nvidia-dynamo-agentic-kv-readwrite-2026.webp)
...
![Disaggregated serving transfer control plane](assets/nvidia-k8s-disagg-serving-2026.webp)
...
![FluxMoE architecture and expert residency](assets/extracted/fluxmoe-01.png)
...
![Wide expert parallelism on rack-scale systems](assets/nvidia-wide-ep-moe-2025.webp)
...
| 判断 | 直接支撑材料 | 关键数字或图 |
| --- | --- | --- |
| FluxMoE、FineMoE、SpecMoEOff、PROBE 分别对应驻留解耦、细粒度预取、延迟隐藏和实时预测调度四种路线 | `S008 (FluxMoE) S036 (FineMoE) S037 (SpecMoEOff) S048 (PROBE)` | FluxMoE `3.0x`；FineMoE 延迟 `-47%` / hit rate `+39%`；SpecMoEOff `2.5x`；PROBE `1.32×` prefill / `1.26×` decode |
| MoE 的关键不是“缺哪个专家就搬哪个”，而是持续治理 route -> place -> move -> overlap -> rebalance 链 | `S008 (FluxMoE) S036 (FineMoE) S037 (SpecMoEOff) S048 (PROBE)` | decoupled expert residency；trajectory-guided prefetch；speculative overlap；dual-track hidden scheduling |
| CPU 的新职责是维护驻留视图、热点预测和同步窗口，而非被动搬运 | `S008 (FluxMoE) S036 (FineMoE) S037 (SpecMoEOff) S048 (PROBE)` | residency tiers；expert map；latency hiding；real-time predictor + solver trigger |
...
- `PROBE` 回答“能否通过实时预测 + 双轨流水线，把预取和重平衡同时隐藏进主路径”。

四者共同说明，MoE 的 host-side orchestration 已经不再是粗糙的“缺哪个搬哪个”，而是在向更完整的 residency control 演化。PROBE 的加入尤其重要，因为它把问题从"decoding 阶段的优化"扩展到了"prefill + decoding 全阶段的实时控制"。

### 6. 小结

本节真正要立住的判断是：MoE 的下一代收益并不主要来自更聪明的 gate，而来自更聪明的驻留、预取与重叠。FluxMoE 的 `3.0x`、FineMoE 的延迟 `-47%` / hit rate `+39%`、SpecMoEOff 的 `2.5x`，以及 PROBE 的 `1.32×` / `1.26×` 共同支撑一个稳健结论：**AI CPU 已经被推到需要持续维护专家状态图、热点预测与同步窗口的位置，并且这种维护正在从"历史统计"升级为"实时预测调度"。** 下一章再看更现实的一层问题：工业界究竟已经吸收到哪一步。[1][2][3][4]
...
![Industrial wide expert parallelism](assets/nvidia-wide-ep-moe-2025.webp)
...
![CPU-centric agentic workflow](assets/cpu-centric-agentic-workflow.png)
...
![Agentic tool call prefill and offload workflow](assets/agentic-tool-call-offload-prefetch.svg)
...
![Disaggregated prefill/decode overview](assets/vllm-disagg-prefill-overview.jpg)
...
![Disaggregated serving on Kubernetes](assets/nvidia-k8s-disagg-serving-2026.webp)
...
![CPU-GPU unified memory and transfer stack](assets/cpu-gpu-unified-memory.webp)
