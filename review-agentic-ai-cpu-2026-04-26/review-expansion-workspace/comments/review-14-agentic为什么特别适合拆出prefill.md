...
处理结论：接受。
说明：这是 `14` 当前最明显的问题，前半段未编号正文与后半段正式结构并存。
修改记录：计划采用方案 A，以带判断表的正式版本为骨架，吸收前半段中更完整的 CPU 复杂度论述后删除重复正文。
...
处理结论：接受。
说明：这条建议成立。`14` 这里现在还是结论先行，技术现实性的铺垫不够。
修改记录：计划补一小段，把 reduced-KV / hybrid attention、prefix reuse / early reuse 与 remote prefill 可行性连起来。
...
处理结论：接受。
说明：评论成立，重点不在换图，而在于把图和本章判断钉紧。
修改记录：计划重写图 1 图注，明确指出图中反复出现的 prefill / resume / prefix re-enter 对本章判断的支撑。
