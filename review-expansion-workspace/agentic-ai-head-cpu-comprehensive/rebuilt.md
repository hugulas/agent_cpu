...
最近的 `Prefill-as-a-Service` 工作进一步把这条证据链向前推进了一步：它不再把 `prefill/decode disaggregation` 理解为单集群内的部署技巧，而是将其扩展到跨数据中心、异构集群和商品以太网络上的 `selective prefill offloading`。这意味着机头 CPU 的职责不再只是本地阶段切换，而要进一步承担带宽感知调度、prefix cache 感知放置以及跨域 KVCache 传输协调 [31]。
...
如果说 NVIDIA 的解耦 serving 文章表明 `prefill/decode` 已经在单集群内成为独立阶段，那么 Kimi/Moonshot 的 `Prefill-as-a-Service` 则说明这条链路正在继续外延。该工作指出，混合注意力模型降低 KVCache 体积后，长上下文 prefill 可以被选择性卸载到独立、计算密集的远端 prefill 集群，再通过商品以太网把生成的 KVCache 回传给本地 decode 集群；但真正的难点不在“能否传回 KVCache”，而在 `bursty workload`、请求长度偏斜、prefix cache 分布不均和跨集群带宽波动下，机头 CPU 是否还能做对 `bandwidth-aware scheduling` 与 `cache-aware request placement` [31]。

这条证据很重要，因为它把本文对 CPU 的判断又往前推了一层。过去讨论调度墙时，默认控制动作主要发生在单节点或单高带宽域内；而 `Prefill-as-a-Service` 表明，一旦 prefill 被真正做成服务，CPU 就不仅要决定“下一步在哪张 GPU 上跑”，还要决定“哪些请求值得被远端 prefill、何时跨域传输 KVCache、哪些 prefix cache 应留在本地以避免回传抖动”。换句话说，agentic inference 下的 CPU 问题正在从 `intra-cluster orchestration` 扩展到 `cross-cluster orchestration`。
...
5. `remote prefill service node`：更看重长上下文 prefill 吞吐、跨域带宽感知调度、prefix cache 感知放置与 KV 回传稳定性 [31]。  
...
`Prefill-as-a-Service` 进一步说明，未来的节点角色不只是在单数据中心内部拆分。只要长上下文 prefill 可以被选择性外包到远端、计算更密集的集群，部署问题就会从“如何在本地拆 ingress / prefill / decode”扩展到“如何定义跨域 prefill 服务节点与本地 decode 节点之间的职责边界”。这意味着 CPU 选型和网络规划要同时回答两个问题：远端 prefill 节点能否稳定消化长上下文 burst，本地 decode 侧又能否在带宽波动和 prefix cache 偏斜下维持低抖动恢复 [31]。
...
| Prefill-as-a-Service | 吞吐相对同构 PD 部署提升 54%，相对朴素异构基线提升 32% | PD 分离正从单集群内优化扩展到跨数据中心、带宽感知与 cache-aware 的 CPU 协调问题 | [31] |
...
[31] QIN R, HE W, WANG Y, et al. Prefill-as-a-Service: KVCache of next-generation models could go cross-datacenter[EB/OL]. arXiv:2604.15039, 2026[2026-04-25]. https://arxiv.org/abs/2604.15039.
